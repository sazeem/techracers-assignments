...
exports.getPullRequestList = (req,res) => {
  ...
  pullRequests.findAll()
   .then((pullRequests) => requestGitHub(pullRequests,token,login,repoName,res));
}

const requestGitHub = (pullRequests,token,login,repoName,res) => {
  let repoList = [];
  let ownerList = [];
...
const storePullRequests = (repos,myResponse,repoName,res) => { 
  let myPullRequests = mapper(myResponse,repos,repoName);
