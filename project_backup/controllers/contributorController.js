...
exports.getContributorList = (req,res) => {
  ...
  contributors.findAll()
   .then((contributors) => requestGitHub(contributors,token,login,repoName,res) );
}

const requestGitHub = (contributors,token,login,repoName,res) => { 
  let repoList = [];
  let ownerList = [];
...
const storeContributors = (repos,myResponse,repoName,res) => {
  let myContributors = mapper(myResponse,repos,repoName);
